...
const (
	authServerURL = "http://localhost:23333/oauth2"
)
...
func main() {
	...
	http.HandleFunc("/try", func(w http.ResponseWriter, r *http.Request) {
		...
		resp, err := http.Get(fmt.Sprintf("http://localhost:23333/api/data?access_token=%s&username=timber3252&scope=userinfo", globalToken.AccessToken))
		if err != nil {
